...
public class BibliotecaService {
  ...
  public void actualizaAutoresPreferidos(Set<String> autoresFavoritos) {
    ...
    writeLock.lock();
    try {
      List<Autor> autoresActualizados = autores.stream()
          .map(autor -> {
  ...
  public void actualizaIdiomasFavoritos(Set<String> idiomasFavoritos) {
    ...
    writeLock.lock();
    try {
      List<Idioma> idiomasActualizados = idiomas.stream()
          .map(idioma -> {
  ...
  public void actualizaGenerosFavoritos(Set<String> generosFavoritos) {
    ...
    writeLock.lock();
    try {
      List<Genero> generosActualizados = generos.stream()
          .map(genero -> {
  ...
  private BiFunction<String, String, List<Integer>> SEARCH_AND_UDPATE_BOOK = (titulo,
      autores) -> {
    ...
    try (ResultSet<Libro> queryResult = libreria
        .retrieve(BusquedaLibro.getPartialQuery(Libro.tituloComparable(titulo), Libro.LIBRO_TITULO_COMPARABLE))) {
      queryResult.stream()
          .forEach(libro -> {
