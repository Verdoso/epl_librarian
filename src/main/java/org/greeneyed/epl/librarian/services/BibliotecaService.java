...
public class BibliotecaService {
  ...
  public void actualizaLibrosDescartados(Set<Integer> libros) {
    ...
    try {
      libreria.stream()
          .filter(libro -> libros.contains(libro.getId()))
          .forEach(libro -> {
            libreria.remove(libro);
  ...
  public void actualizaAutoresPreferidos(Set<String> autoresFavoritos) {
    ...
    try {
      autores.stream()
          .filter(autor -> autoresFavoritos.contains(autor.getNombre()))
          .forEach(autor -> {
  ...
  public void actualizaIdiomasFavoritos(Set<String> idiomasFavoritos) {
    ...
    try {
      idiomas.stream()
          .filter(idioma -> idiomasFavoritos.contains(idioma.getNombre()))
          .forEach(idioma -> {
  ...
  public void actualizaGenerosFavoritos(Set<String> generosFavoritos) {
    ...
    try {
      generos.stream()
          .filter(genero -> generosFavoritos.contains(genero.getNombre()))
          .forEach(genero -> {
  ...
  private TriFunction<String, String, CalibreService.CALIBRE_LANGUAGE, List<Integer>> SEARCH_AND_UDPATE_BOOK = (titulo,
      autores,
      calibreLanguage) -> {
