...
public class BibliotecaService {
  ...
  public enum IDIOMA_ORDERING implements ElementOrdering<Idioma> {
    POR_IDIOMA(queryOptions(orderBy(ascending(Idioma.IDIOMA_NOMBRE))), queryOptions(orderBy(descending(Idioma.IDIOMA_NOMBRE)))),
    POR_LIBROS(queryOptions(orderBy(ascending(Idioma.IDIOMA_LIBROS))), queryOptions(orderBy(descending(Idioma.IDIOMA_LIBROS))));
  ...
  public enum GENERO_ORDERING implements ElementOrdering<Genero> {
    POR_GENERO(queryOptions(orderBy(ascending(Genero.GENERO_NOMBRE))), queryOptions(orderBy(descending(Genero.GENERO_NOMBRE)))),
    POR_LIBROS(queryOptions(orderBy(ascending(Genero.GENERO_LIBROS))), queryOptions(orderBy(descending(Genero.GENERO_LIBROS))));
  ...
  public enum AUTOR_ORDERING implements ElementOrdering<Autor> {
    POR_AUTOR(queryOptions(orderBy(ascending(Autor.AUTOR_NOMBRE))), queryOptions(orderBy(descending(Autor.AUTOR_NOMBRE)))),
    POR_LIBROS(queryOptions(orderBy(ascending(Autor.AUTOR_LIBROS))), queryOptions(orderBy(descending(Autor.AUTOR_LIBROS))));
  ...
  public enum BOOK_ORDERING implements ElementOrdering<Libro> {
    POR_TITULO(queryOptions(orderBy(ascending(Libro.LIBRO_TITULO))), queryOptions(orderBy(descending(Libro.LIBRO_TITULO))))
    //
    ...
    //
    , POR_COLECCION(queryOptions(orderBy(ascending(Libro.LIBRO_COLECCION), ascending(Libro.LIBRO_VOLUMEN), ascending(Libro.LIBRO_TITULO))),
        queryOptions(orderBy(descending(Libro.LIBRO_COLECCION), descending(Libro.LIBRO_VOLUMEN), descending(Libro.LIBRO_TITULO))))
    //
    ;
  ...
  public Sumario getSumario() {
    if (sumario == null) {
      ...
      int numIdiomas = (int) libreria.stream()
          .map(Libro::getIdioma)
          .distinct()
          .count();
      int numAutores = (int) libreria.stream()
          .map(Libro::getAutor)
          .distinct()
          .count();
      int numGeneros = (int) libreria.stream()
          ...
          .distinct()
          .count();
      sumario = new Sumario(buildVersion, fechaActualizacion.atZone(ZoneId.systemDefault())
          .toInstant()
          .toEpochMilli(), numLibros, numIdiomas, numAutores, numGeneros, calibreService.isEnabled());
    }
    return sumario;
  }
  ...
  private <O extends PuedeSerFavorito> Pagina<O> paginaElementos(IndexedCollection<O> elementos,
      ...
      Function<String, Boolean> esFavorito) {
    ...
    try (final ResultSet<O> queryResult = elementos.retrieve(busqueda.getQuery(), busqueda.getQueryOptions())) {
      if (busqueda.isSoloFavoritos()) {
        ...
        pagina.setTotal((int) queryResult.stream()
  ...
  public void actualizaAutoresPreferidos(Set<String> autores) {
    ...
    try {
      libreria.stream()
          .forEach(libro -> libro.setAutorFavorito(false));
  ...
  public void actualizaIdiomasFavoritos(Set<String> idiomas) {
    log.info("Actualizando idiomas preferidos...");
    writeLock.lock();
    try {
      libreria.stream()
          .forEach(libro -> libro.setIdiomaFavorito(false));
      Set<String> normalisedIdiomas = idiomas.stream()
          .map(Libro::flattenToAscii)
          .collect(Collectors.toSet());
  ...
  public void actualizaGenerosFavoritos(Set<String> generos) {
    log.info("Actualizando generos preferidos...");
    writeLock.lock();
    try {
      libreria.stream()
          .forEach(libro -> libro.setGeneroFavorito(false));
      Set<String> normalisedGeneros = generos.stream()
          .map(Libro::flattenToAscii)
          .collect(Collectors.toSet());
  ...
  private BiConsumer<String, String> SEARCH_AND_UDPATE_BOOK = (titulo,
      autores) -> {
    ...
    Set<String> autoresSet = Arrays.asList(autores.split(" & "))
        ...
        .map(Libro::flattenToAscii)
        .collect(Collectors.toSet());
    try (ResultSet<Libro> queryResult = libreria.retrieve(BusquedaLibro.getPartialQuery(Libro.flattenToAscii(titulo), Libro.LIBRO_TITULO))) {
      queryResult.stream()
          .forEach(libro -> {
    ...
    List<Autor> topAutores = autores.stream()
        ...
        .map(autor -> {
          try (ResultSet<Libro> queryResult = libreria
              .retrieve(and(in(Libro.LIBRO_AUTORES, Libro.flattenToAscii(autor.getNombre())), equal(Libro.LIBRO_EN_CALIBRE, Boolean.TRUE)))) {
            return new Autor(autor.getNombre(), queryResult.size());
          }
        })
        .sorted(Comparator.comparing(Autor::getLibros)
    ...
    List<Idioma> topIdiomas = idiomas.stream()
        .map(idioma -> {
          try (final ResultSet<Libro> queryResult = libreria
              .retrieve(and(equal(Libro.LIBRO_IDIOMA, Libro.flattenToAscii(idioma.getNombre())), equal(Libro.LIBRO_EN_CALIBRE, Boolean.TRUE)))) {
            return new Idioma(idioma.getNombre(), queryResult.size());
          }
        })
        .sorted(Comparator.comparing(Idioma::getLibros)
            .thenComparing(Idioma::getNombre)
            .reversed())
  ...
  public Pagina<Genero> getTopGeneros(int porPagina) {
    List<Genero> topGeneros = generos.stream()
        .map(genero -> {
          try (final ResultSet<Libro> queryResult = libreria
              .retrieve(and(in(Libro.LIBRO_GENEROS, Libro.flattenToAscii(genero.getNombre())), equal(Libro.LIBRO_EN_CALIBRE, Boolean.TRUE)))) {
            return new Genero(genero.getNombre(), queryResult.size());
          }
        })
        .sorted(Comparator.comparing(Genero::getLibros)
            .thenComparing(Genero::getNombre)
            .reversed())
