  ...
  public void update(UpdateSpec updateSpec) {
    ...
    try {
      ...
      autores.addAll(libreria.stream()
          ...
          .distinct()
          .map(nombre -> {
            try (final ResultSet<Libro> queryResult = libreria.retrieve(contains(Libro.LIBRO_AUTOR, Libro.flattenToAscii(nombre)))) {
      ...
      generos.addAll(libreria.stream()
          ...
          .distinct()
          .map(nombre -> {
            try (final ResultSet<Libro> queryResult = libreria.retrieve(contains(Libro.LIBRO_GENERO, Libro.flattenToAscii(nombre)))) {
